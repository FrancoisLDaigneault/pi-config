...
export default function (pi: ExtensionAPI) {
	activate(pi, { askClaudeTool: true });
}

// Shared activation body. The provider-only entry (src/provider-only.ts) runs
// everything here except the AskClaude tool registration, so locked-down
// agents can use claude-bridge models without also receiving a tool that can
// launch Claude Code with write and bash access.
export function activate(pi: ExtensionAPI, opts: { askClaudeTool: boolean }) {
	...
	if (askConf?.enabled && opts.askClaudeTool) {
