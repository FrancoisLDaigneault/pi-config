...
Context7 is a hosted service and the MCP adapter caches tool metadata, so no local
fetch cache is required. The package ships an optional ETag helper at
`scripts/lib/doc-fetch-cache.sh` (relative to the bigpowers package root). It needs
`shasum` on PATH and writes to `.bigpowers/cache/docs` relative to the **current
working directory**, so enable it deliberately with `DOC_CACHE_DIR` set to an
absolute path outside your repositories.
