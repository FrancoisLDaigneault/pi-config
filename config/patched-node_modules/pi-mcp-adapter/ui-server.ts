  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === "/proxy/tools/call") {
        ...
        const activeConfig = options.config ?? options.state?.config;
        const definition = activeConfig?.mcpServers[options.serverName] ?? connection.definition;
        if (!isToolAllowed(
          callParams.name,
          options.serverName,
          resolveToolPrefix(definition, activeConfig?.settings?.toolPrefix),
          definition.includeTools,
          definition.excludeTools,
        )) {
          sendJson(res, 403, { ok: false, error: `MCP tool "${callParams.name}" is blocked by server configuration` });
          return;
        }
