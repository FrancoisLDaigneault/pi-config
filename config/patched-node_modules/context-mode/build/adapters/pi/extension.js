...
export default function piExtension(pi) {
    ...
    pi.on("before_agent_start", async (event, ctx) => {
        try {
            ...
            const _isForeground = isForegroundSession(ctx);
            await ensureMCPBridge(_isForeground);
            if (!_sessionId)
                return;
            ...
            // LOCAL PATCH — gate the routing anchor on foreground.
            // Subagents (hasUI:false) have a closed tool allowlist declared in their
            // agent frontmatter (e.g. pi-subagents/agents/oracle.md ->
            // `tools: read, grep, find, ls, bash, intercom`), so NO ctx_* tool is
            // registered for them. Injecting this anchor there instructs a subagent to
            // route work through tools that do not exist in its schema. The foreground
            // flag is already computed above for ensureMCPBridge(); reuse it.
            if (_isForeground) {
